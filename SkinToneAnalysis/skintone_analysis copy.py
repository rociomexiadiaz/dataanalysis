...
def plot_architecture_analysis(df_dict, save_path='./'):
    """Plot architecture analysis charts showing differences from Baseline"""
    ...
    # Get the Baseline values for each skin tone
    baseline_data = grouped[grouped['Model'] == 'Baseline']
    if baseline_data.empty:
        print("No Baseline data found.")
        return
    
    # Create a mapping of skin tone to baseline values
    baseline_map = {}
    for _, row in baseline_data.iterrows():
        baseline_map[row['Skin Tone']] = row['mean']
    
    # Filter out Baseline from the models and calculate differences
    other_models = grouped[grouped['Model'] != 'Baseline']
    
    if other_models.empty:
        print("No other model data found.")
        return
    ...
    for _, row in other_models.iterrows():
        ...
        if skin_tone in baseline_map:
            diff = row['mean'] - baseline_map[skin_tone]
            differences.append({
                'Model': row['Model'],
                'Skin Tone': skin_tone,
                'Difference': diff,
                'std': row['std']  # Keep the original std for error bars
            })
    ...
    for i, tone in enumerate(skin_tones):
        ...
        errors = []
        for model in models:
            row = diff_df[(diff_df['Model'] == model) & (diff_df['Skin Tone'] == tone)]
            if not row.empty:
                values.append(row['Difference'].iloc[0])
    ...
    plt.title('Architecture Sensitivity: Difference from Baseline')
    ...
    # Keep the existing disparity plots unchanged (including Baseline)
...
def plot_dataset_analysis(df_dict, save_path='./'):
    """Plot dataset analysis charts showing differences from 'all' baseline"""
    ...
    # Get the 'all' baseline values for each skin tone
    all_baseline = grouped[grouped['Datasets'] == 'all']
    if all_baseline.empty:
        print("No 'all' baseline data found.")
        return
    
    # Create a mapping of skin tone to 'all' baseline values
    all_baseline_map = {}
    for _, row in all_baseline.iterrows():
        all_baseline_map[row['Skin Tone']] = row['mean']
    
    # Filter out 'all' from the datasets and calculate differences
    other_datasets = grouped[grouped['Datasets'] != 'all']
    
    if other_datasets.empty:
        print("No other dataset data found.")
        return
    
    # Calculate differences from 'all' baseline (all - minus_x)
    differences = []
    for _, row in other_datasets.iterrows():
        skin_tone = row['Skin Tone']
        if skin_tone in all_baseline_map:
            diff = all_baseline_map[skin_tone] - row['mean']  # all - minus_x
            differences.append({
    ...
    dataset_name_mapping = {
        'minus_dermie': 'Dermie',
        'minus_fitz': 'Fitzpatrick17k',
        'minus_india': 'India',
        'minus_pad': 'PADUFES',
        'minus_scin': 'SCIN'
    }
    
    datasets = diff_df['Datasets'].unique()
    display_names = [dataset_name_mapping.get(dataset, dataset) for dataset in datasets]
    skin_tones = [1,2,3,4,5,6]
    ...
    for i, tone in enumerate(skin_tones):
        ...
        for dataset in datasets:
            row = diff_df[(diff_df['Datasets'] == dataset) & (diff_df['Skin Tone'] == tone)]
            if not row.empty:
                values.append(row['Difference'].iloc[0])
                errors.append(row['std'].iloc[0])
            else:
    ...
    plt.xticks(x + width * (len(skin_tones) - 1) / 2, display_names, rotation=45, ha='right')
    plt.ylabel('Performance Drop from "All" Baseline (%)')
    plt.title('Dataset Sensitivity: Performance Drop from "All" Baseline')
    plt.grid(True, alpha=0.3)
    ...
    # Keep the existing disparity plots unchanged (including 'all')
